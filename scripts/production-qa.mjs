...
if (production && (!siteUrl?.startsWith('https://') || siteUrl.includes('example.invalid'))) {
  fail('production site URL must be a real HTTPS origin.');
}
if (production && !indexable) warn('production asset is still non-indexable; this is allowed before OPEN_FOR_INDEXING.');
...
for (const path of htmlFiles) {
  ...
  if (production && canonical?.includes('example.invalid')) fail(`${label}: production canonical uses example.invalid.`);
  if (!production && !/name=["']robots["'][^>]*content=["']noindex,\s*follow["']/i.test(html)) {
    fail(`${label}: planned/staging output must be noindex, follow.`);
  }
  if (production && /\b(TBD|CHANGEME|example\.invalid)\b/i.test(html)) fail(`${label}: production output contains placeholder content.`);
}
...
for (const path of htmlFiles) {
  ...
  for (const href of hrefs) {
    ...
    const exists = await stat(candidate).then(() => true).catch(() => false)
      || await stat(fallback).then(() => true).catch(() => false);
    if (!exists) fail(`${relative('dist', path)}: broken internal link ${href}`);
  }
}

if (failed) process.exit(1);
console.log(`Production QA PASS (${production ? 'production' : 'non-production'} mode): rendered metadata, indexability, sitemap, robots, and internal links validated.`);
